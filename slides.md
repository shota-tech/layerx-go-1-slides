...
### → ビルド時にプロファイルを与えることで、それを判断できるようになる！
...
- 公式ドキュメント曰く 2~14% のパフォーマンス向上が見込める。
...
- ビルド時に毎回手動でプロファイルを DL してコミットするのは面倒。
- 自動化して CI/CD に組み込みたい。
...
- PGO により、ソースコードの変更無しで2~14%のパフォーマンス向上が見込める。
